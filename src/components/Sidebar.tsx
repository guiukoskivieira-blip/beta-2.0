...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab = 'dashboard', onSelectTab, disabledTabs = [] }) => {
  const menuItems = [
    ...
    { id: 'verifications', label: 'Verificações', icon: Files },
    { id: 'report', label: 'Relatório técnico', icon: Settings },
  ];

  const handleClick = (id: string) => {
    if (disabledTabs.includes(id)) return; // RBAC: blocked tab
    onSelectTab?.(id);
  };
          ...
          {menuItems.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                title={disabled ? 'Sem permissão para esta ação' : undefined}
                className={`flex min-h-12 w-full items-center gap-3 rounded-xl px-4 text-left text-sm font-bold transition ${
                  active ? 'bg-white/20 text-white shadow-inner ring-1 ring-white/10' :
                  disabled ? 'cursor-not-allowed text-violet-300 opacity-50' :
      ...
      {menuItems.map(({ id, label, icon: Icon }) => {
        ...
        return (
          <button
            ...
            onClick={() => handleClick(id)}
            aria-label={label}
            className={`flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-lg px-1 ${activeTab === id ? 'bg-violet-50 text-violet-700' : disabled ? 'text-slate-300' : 'text-slate-500'}`}
          >
            {disabled ? <Lock className="h-4 w-4" /> : <Icon className="h-4 w-4" />}
            <span className="max-w-full truncate text-[8px] font-bold">{label.split(' ')[0]}</span>
          </button>
        );
      })}
    </nav>
